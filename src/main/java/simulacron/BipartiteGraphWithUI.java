  ...
  entitiesPortrayal.setPortrayalForClass(
      App.class, new RectanglePortrayal2D() {// inline subclass to override draw()
        public void draw(Object object, Graphics2D graphics, DrawInfo2D info) {
          ...
          drawServices(graphics, info.draw, app.services, ((BipartiteGraph)state).getInitServices());
        }
      ...
      Platform.class, new OvalPortrayal2D() {// inline subclass to override draw()
        public void draw(Object object, Graphics2D graphics, DrawInfo2D info) {
          Platform plat = (Platform)object;
					paint = new Color((int)(255 * plat.getSize() / ((BipartiteGraph)state).getInitServices()), 0, 0);
          ...
          info.draw.height = 50;
          super.draw(object, graphics, info);
          drawServices(graphics, info.draw, plat.services, ((BipartiteGraph)state).getInitServices());
        }
      });
...
public void drawServices(Graphics2D graphics, Rectangle2D.Double draw, java.util.List<Service> dna, int numAllServices){
	...
	graphics.drawLine((int)(draw.x), (int)(draw.y - draw.height / 2), (int)(draw.x), (int)(draw.y - draw.height/2 + draw.height * dna.size() / ((BipartiteGraph)state).getInitServices()));
